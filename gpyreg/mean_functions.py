...
class ZeroMean:
    ...
    def hyperparameter_count(self, _):
        return 0

    def hyperparameter_info(self, _):
        return []
    ...
    def compute(self, hyp, X, compute_grad = False):
        ...
        if hyp.size != mean_N:
            raise ValueError('Expected %d mean function hyperparameters, %d passed instead.' % (mean_N, hyp.size))
        if hyp.ndim != 1:
