...
def compute_gradient(f, x0):
    ...
    for i in range(0, np.size(x0)):
        ...
        tmp = sp.misc.derivative(f_i, x0[i], dx=np.finfo(float).eps**(1/5.0), order=5)
        num_grad[i] = tmp
...
def test_gp_gradient_computations():
    ...
    assert np.all(np.isclose(check_grad(f, f_grad, hyp0 * np.exp(0.1*np.random.uniform(size=hyp0.size))), 0.0, atol=1e-6))
