...
def pdf(x, sigma, df, a, b):
    ...
    c = scipy.special.gamma(0.5*(df+1)) / (scipy.special.gamma(0.5*df) * sigma * np.sqrt(df * np.pi))
    C = 1.0 + (b - a) * c
...
def test_pdf():
    ...
    assert np.isclose(1.0, quad(pdf, -np.inf, np.inf, args=(df, sigma, a, b))[0])
    ...
    assert np.isclose(pdf(a-np.spacing(a), df, sigma, a, b), pdf(a+np.spacing(a), df, sigma, a, b))
    assert np.isclose(pdf(b-np.spacing(b), df, sigma, a, b), pdf(b+np.spacing(b), df, sigma, a, b))
...
def test_cdf_ppf():
    ...
    for x in xx:
        assert np.isclose(smoothbox_student_t_ppf(smoothbox_student_t_cdf(x, df, sigma, a, b), df, sigma, a, b), x)
...
def test_ppf_cdf():
    ...
    for q in qq:
        assert np.isclose(smoothbox_student_t_cdf(smoothbox_student_t_ppf(q, df, sigma, a, b), df, sigma, a, b), q)
