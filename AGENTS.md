...
- `GP.fit(X, y, s2, hyp0, options)`: fill unset bounds from the components' recommended bounds, evaluate the objective on a Sobol space-filling design of initial candidates built by `f_min_fill` (which maps unit-cube points through the hyperparameter priors), run `scipy.optimize.minimize` with analytic gradients from the best candidates, optionally run `SliceSampler` for `n_samples` posterior draws, then call `update` to build posteriors. Returns `(hyp, optimize_result, sampling_result)`. A fit that raises restores the GP from a shallow copy of its `__dict__`, taken after the fit has checked its arguments and computed its bounds and before it changes anything. The code that a fit runs therefore replaces an attribute of the GP that it changes and never writes into the object that the attribute held before the fit (the data, the bounds, the normalization constants, the posteriors): such a write survives the restore. `hyper_priors["df"]`, which the fit fills for its duration and puts back itself, is the one exception.
